...
// 2. Redesign Hero Header properly
...
// Additional fixes for AdminEventList items rendering
content = content.replace(
  /<View style=\{styles\.eiThumb\}>[\s\S]*?<\/View>/g,
  `<View style={styles.eiThumb}>
              {item.imageUrl ? (
                <Image source={{ uri: item.imageUrl }} style={styles.eiThumbImg} />
              ) : (
                <Text style={styles.eiThumbTxt}>NO IMG</Text>
              )}
            </View>`
);

content = content.replace(
  /<Text style=\{styles\.eiTitle\} numberOfLines=\{1\}>\{item\.title\}<\/Text>\s*<Text style=\{styles\.eiTe\} numberOfLines=\{1\}>\{item\.titleTe || 'తెలుగు టైటిల్ లేదు'\}<\/Text>/,
  `<Text style={styles.eiTitle} numberOfLines={1}>{item.title}</Text>
            <Text style={styles.eiTe} numberOfLines={1}>{item.titleTe || 'తెలుగు టైటిల్ లేదు'}</Text>`
);

content = content.replace(
  /<Text style=\{styles\.badgePubTxt\}>PUBLISHED<\/Text>/g,
  `<Text style={styles.badgePubTxt}>PUBLISHED</Text>`
);
