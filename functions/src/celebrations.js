...
async function runDailyCelebrationsSweep() {
    ...
    for (const churchDoc of churchesSnap.docs) {
        ...
        for (const celebrant of celebrantsList) {
            ...
            let emoji = '🎉';
            if (celebrant.type === 'Birthday')
                emoji = '🎂';
            if (celebrant.type === 'Wedding Anniversary')
                emoji = '💍';
            if (celebrant.type === 'Baptism Anniversary')
                emoji = '💧';
...
export const weCelebrationWishCreatedTrigger = functions.region('asia-south1').firestore.document('churches/{churchId}/live_celebrations/{dateId}/messages/{msgId}').onCreate(async (snap, context) => {
    ...
    if (!targetCelebrantId)
        return; // Not targeted to a specific celebrant
...
export const weCelebrationBatchedWishes = functions.pubsub.schedule('*/10 * * * *').onRun(async (context) => {
